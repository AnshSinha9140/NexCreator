  ...
  const handleAction = async (id: string, action: "approve" | "reject" | "request_changes" | "suspend" | "ban", note?: string) => {
    try {
      const res = await fetch("/api/admin/creators", {
        ...
        body: JSON.stringify({ creatorId: id, action, notes: note }),
      });
      const json = await res.json();
      if (json.success) {
  ...
  return (
    <>
      <AdminHeader
        ...
        subtitle="Review, Approve, or Audit Platform Verification Applications"
      ...
      <div className="admin-page">
        ...
        {/* Creator List with explicit flex gap margin */}
        {loading ? (
